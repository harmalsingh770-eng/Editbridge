...
export default function Admin() {
  ...
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, (u) => {
      ...
      unsubEdRef.current = onSnapshot(collection(db, "editors"), (snap) => {
      ...
      unsubChatsRef.current = onSnapshot(collection(db, "chats"), (snap) => {
    ...
    return () => {
      ...
      if (unsubEdRef.current) unsubEdRef.current();
  ...
  const toggleEditor = async (id, current) =>
      ...
      {showLogout && (
        <div style={s.overlay}>
          <div style={s.modal}>
            ...
            <p style={{ color: "#94a3b8", fontSize: 14, margin: "0 0 20px" }}>Do you really want to logout?</p>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setShowLogout(false)} style={s.mCancel}>Stay</button>
              <button onClick={confirmLogout} style={s.mConfirm}>Yes, Logout</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={s.page}>
        ...
        <div style={s.header}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button onClick={() => router.push("/")} style={s.btnGray}>Home</button>
        ...
        {/* STATS */}
        <div style={s.stats}>
          <StatCard label="Editors" value={editors.length} color="#7c3aed" />
          <StatCard label="Users" value={users.length} color="#3b82f6" />
          <StatCard label="Pending" value={pending.length} color="#f59e0b" />
          <StatCard label="Chats" value={allChats.length} color="#10b981" />
        </div>
              ...
              : filteredPayments.map(p => (
                <div key={p.id} style={s.card}>
                  ...
                  <div style={s.cardActions}>
                    ...
                    <button onClick={() => openChat(p.uid, p.email || "Client")} style={s.btnPurple}>
                      Chat
                    </button>
                  </div>
                </div>
              ...
              : filteredEditors.map(e => (
                <div key={e.id} style={s.card}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={s.cardSub}>Price: Rs.{e.price || 0}</div>
                  </div>
                  <div style={s.cardActions}>
                    <span style={statusBadge(e.approved ? "approved" : "pending")}>
                      {e.approved ? "approved" : "pending"}
                    </span>
                    {!e.approved && (
                      <button onClick={() => approveEditor(e.id)} style={s.btnGreen}>Approve</button>
                    )}
                    <button onClick={() => toggleEditor(e.id, e.active)} style={s.btnBlue}>
                      {e.active ? "Disable" : "Enable"}
                    </button>
                    <button onClick={() => openChat(e.id, e.name || "Editor")} style={s.btnPurple}>
                      Chat
                    </button>
                    <button onClick={() => deleteEditor(e.id)} style={s.btnRed2}>Delete</button>
                  </div>
                </div>
              ))
          )}
              ...
              : allChats.map(c => (
                <div key={c.id} style={s.card}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={s.cardSub}>ID: {c.id}</div>
                    <div style={s.cardSub}>
                      Last: {c.lastMessage || "No messages"}
                    </div>
                  </div>
                  <button
...
const s = {
  ...
  header: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px", borderBottom: "1px solid rgba(255,255,255,0.08)", flexWrap: "wrap", gap: 8 },
  ...
  cardActions: { display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" },

  btnGray: { background: "#334155", color: "white", padding: "7px 12px", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 13 },
  btnRed: { background: "#ef4444", color: "white", padding: "7px 12px", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 13 },
  btnGreen: { background: "#10b981", color: "white", padding: "6px 10px", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 12 },
  btnBlue: { background: "#3b82f6", color: "white", padding: "6px 10px", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 12 },
  ...
  chatPanel: { position: "fixed", bottom: 0, right: 0, width: "min(340px, 100vw)", height: 440, background: "#0f172a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "16px 16px 0 0", display: "flex", flexDirection: "column", zIndex: 200, boxShadow: "0 -4px 30px rgba(0,0,0,0.6)" },
  ...
  chatMsg: { padding: "9px 13px", borderRadius: 12, maxWidth: "80%", fontSize: 14, color: "white", wordBreak: "break-word" },
